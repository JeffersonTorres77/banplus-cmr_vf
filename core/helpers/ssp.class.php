...
define('DATATABLE_DETAILS', [
	"type" => "Sqlserver",
	"server_version" => 2019,
	'user' => config('DB.USUARIO'),
	'pass' => config('DB.CLAVE'),
	'db'   => config('DB.NOMBRE'),
	'host' => config('DB.HOST')
]);
...
class SSP
{
	 ...
	 */
	static function simple($request, $table, $primaryKey, $columns, $conn = DATATABLE_DETAILS)
	{
		$bindings = array();
		$db = self::db($conn);
	 ...
	 *  @return array          Server-side processing response array
	 */
	static function complex($request, $table, $primaryKey, $columns, $whereResult = null, $whereAll = null, $conn = DATATABLE_DETAILS)
	{
		$bindings = array();
		$db = self::db($conn);
